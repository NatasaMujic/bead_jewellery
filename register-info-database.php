<?php
//php code for insert data about registered user into database
...
$streetNumber = $_POST['street_num'];
$zip = $_POST['zip'];
$password = $_POST['password'];
$confPass = $_POST['conf_password'];
$day_of_birth = $_POST['day_of_birth'];
$checkbox_terms_of_use=$_POST['checkbox_terms_of_use'];

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($firstName) && isset($lastName) && isset($email) && isset($phoneNum) && isset($state) && isset($city) && isset($address) && isset($streetNumber)
    && isset($zip) && isset($password) && isset($confPass) && isset($day_of_birth) && isset($checkbox_terms_of_use)){

        if(empty($firstName) || empty($lastName) || empty($email) || empty($phoneNum) || empty($state) || empty($city) || empty($address) ||
        empty($streetNumber) || empty($zip) || empty($password) || empty($confPass) || empty($day_of_birth) || !isset($_POST['checkbox_terms_of_use'])){
            ...
            //check if the user with that email already exist into database
           $stmt=$pdo->prepare("SELECT * FROM user_registered WHERE email=:email");
           ...
           if($existingUser){
               ...
               $stmt=$pdo->prepare("INSERT INTO user_registered (first_name,last_name,email,password,phone_number,state,city,address,street_number,
               zip_code,date_of_birth) VALUES (:firstName,:lastName,:email,:hashedPassword,:phoneNum,:state,:city,:address,:streetNumber,:zip,:day_of_birth)");
        ...
        echo "Some data are not entered!";
    }
}else{
    echo "Some error occupied!";
}
?>
<!DOCTYPE html>
<html>
...
<div id="header" class="container-fluid vh-100">
    <div id="navbar" class="navbar">
        ...
        <a href="#"><img src="index-images/wishlist.png" alt="image" style=" width: 35px; height: 35px; position: relative;"></a>
        <a href="#"><img src="index-images/add-cart.png" alt="image" style=" width: 35px; height: 35px; position: relative;"></a>
        <a href="#" style="text-decoration: none; color: black;"><li>SIGN OUT</li></a>
